...
\begin{table}
    ...
    \begin{tabular}{l l l}
        \hline
        \multirow{2}{*}{\bfseries Name} & \multicolumn{2}{l}{\bfseries Magnetic Field (\textmu G)} \\
        & KS-EDF & $\sigma$-Sub. \\
        \hline
        \csvreader[head to column names]{"../../Resources/CSV/results_filtered_proc.csv"}{}
        {\\\csvcoli & \csvcolvi & \csvcolxi}
        \\
        \hline
    \end{tabular}
    \caption{A table describing the magnetic field derivations for each HVC.}
    \label{tab:Bdev}
\end{table}
...
\begin{figure}
    ...
    \centering
    \caption{A boxplot representing the absolute-value magnetic field of each HVC for both derivational methods. The red and blue dotted lines indicate two upper bounds, one set by the Grønnow Simulations, and the other by the Smith Cloud respectively.}
    \label{fig:BBox}
\end{figure}
...
\begin{figure}
    \includegraphics[width=10cm]{"B_VGSR.png"}
    \centering
...
\begin{figure}
    \includegraphics[width=\textwidth]{"Method_Err_Rel.png"}
    \centering
    \caption{A boxplot representing the relative error between the KS-EDF and Uncertainty Subtraction methods for each HVC. The denominator value is taken to be the KS-EDF value.}
    \label{fig:RelErr}
\end{figure}
